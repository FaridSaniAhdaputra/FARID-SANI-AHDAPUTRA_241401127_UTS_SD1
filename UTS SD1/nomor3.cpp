...
// Deklarasi kelas Graph untuk merepresentasikan graf
class Graph {
private:
    int numVertices;
...
public:
    // Konstruktor: dipanggil saat objek Graph dibuat
    Graph(int V) {
        numVertices = V;
    ...
    //Bagian DFS
    // Fungsi rekursif untuk menjelajahi simpul secara mendalam
    void dfsUtil(int u, vector<bool>& visited, const vector<char>& vertexChar) {
        visited[u] = true;
        cout << vertexChar[u];

        // Periksa semua simpul dari yang terbesar ke terkecil
    ...
    //Bagian BFS
    // BFS digunakan untuk mencari jalur terpendek dalam jumlah edge
...
int main() {
    ...
    // Karakter untuk tiap simpul
    vector<char> vertexChar = {'A', 'B', 'C', 'D', 'E'};
    // Nama tempat yang sesuai dengan masing-masing simpul
